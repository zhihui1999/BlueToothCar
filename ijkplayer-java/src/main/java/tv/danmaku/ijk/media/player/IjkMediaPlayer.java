...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    protected static final int RTCP_SR_DATA = 80000;
    protected static final int TAKE_PIC_STATE = 80010;  /* arg1 = undefined, arg2 = result code, obj = file name*/
                                                        /* arg2: < 0, error occurred, = 0, succeed and continue, = 1, succeed and finished */
    protected static final int REC_VIDEO_STATE = 80020;
    protected static final int DEVICE_CONNECTED = 80030;
    ...
    public static final int FFP_PROP_INT64_RTP_SPEED                        = 40200;
    public static final int FFP_PROP_INT64_RTP_TRAFFIC_STATISTIC_BYTE_COUNT = 40204;
    public static final int FFP_PROP_INT64_CONNECTED_TIME                   = 40300;
    //----------------------------------------
    ...
    public void sendRtcpRrData(byte[] data) throws IllegalStateException {
        _sendRtcpRrData(data, data.length);
    }

    private native void _sendRtcpRrData(byte[] data, int dataLen) throws IllegalStateException;

    @Override
    public void takePicture(String path, String fileName, int width, int height, int number)
            throws IOException, IllegalArgumentException, SecurityException, IllegalStateException {
        _takePicture(path, fileName, width, height, number);
    }

    private native void _takePicture(String path, String fileName, int width, int height, int number)
            throws IOException, IllegalArgumentException, SecurityException, IllegalStateException;

    @Override
    public void startRecordVideo(String path, String fileName, int width, int height)
            throws IOException, IllegalArgumentException, SecurityException, IllegalStateException {
        _startRecordVideo(path, fileName, width, height);
    }

    private native void _startRecordVideo(String path, String fileName, int width, int height)
            throws IOException, IllegalArgumentException, SecurityException, IllegalStateException;
    ...
    private native void _stopRecordVideo() throws IllegalStateException;

    @Override
    public boolean setVrMode(boolean vrMode) throws IllegalStateException {
        return _setVrMode(vrMode);
    }

    private native boolean _setVrMode(boolean vrMode) throws IllegalStateException;

    @Override
    public boolean setRotation180(boolean rotation180) throws IllegalStateException {
    ...
    public void setTexcoordRect(float left, float top, float right, float bottom)
            throws IllegalStateException {
        _setTexcoordRect(left, top, right, bottom);
    }

    private native void _setTexcoordRect(float left, float top, float right, float bottom)
            throws IllegalStateException;
    ...
    public long getRtpSpeed() {
        return _getPropertyLong(FFP_PROP_INT64_RTP_SPEED, 0);
    }

    public long getRtpTrafficStatisticByteCount() {
        return _getPropertyLong(FFP_PROP_INT64_RTP_TRAFFIC_STATISTIC_BYTE_COUNT, 0);
    }

    public long getConnectedTime() {
        return _getPropertyLong(FFP_PROP_INT64_CONNECTED_TIME, 0);
    }
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (player == null || player.mNativeMediaPlayer == 0) {
                ...
                DebugLog.w(TAG, "Post events through otto for external handler");
                MBUS.post(msg);
                return;
            }
            ...
            case RTCP_SR_DATA:
                ...
                player.notifyOnReceivedRtcpSrData(data);
                break;

            case TAKE_PIC_STATE:
                // msg.obj is file name
                player.notifyOnTookPicture(msg.arg2, (String)msg.obj);
                break;

            case REC_VIDEO_STATE:
                // msg.obj is file name
                player.notifyOnRecordVideo(msg.arg1, (String)msg.obj);
                break;

            case DEVICE_CONNECTED:
                player.notifyOnDeviceConnected();
                break;
